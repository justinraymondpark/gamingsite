...
// Create a map of old game_id -> rawg_id for reference lookups
const gameIdToRawgId = new Map<number, number>();
...
async function migrate() {
  ...
  for (const note of quickNotes) {
    ...
    if (!rawgId) {
      console.log(`  ✗ Skipping note ${note.id} - no game found for game_id ${note.game_id}`);
      continue;
    }
    ...
    console.log(`  ✓ Note ${note.id} for game ${rawgId}`);
  }
  ...
  for (const review of reviews) {
    ...
    if (!rawgId) {
      console.log(`  ✗ Skipping review ${review.id} - no game found for game_id ${review.game_id}`);
      continue;
    }
    ...
    console.log(`  ✓ Review "${review.title}" for game ${rawgId}`);
  }
